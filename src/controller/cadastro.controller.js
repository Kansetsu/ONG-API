...
controller.post("/cadastrarAcolhido", (req, res) => {
    ...
    res.send(
        cadastroService.create(req.body).catch((error) => {
...
controller.get("/todosOsAcolhidos", async (req, res) => {
    ...
    res.send(await cadastroService.getTodosAcolhidos().catch((error) => {
        console.log(`Ocorreu um erro: ${error}`);
    }));
});
...
controller.get("/acolhidoPorMatricula", async (req, res) => {
    ...
    res.send(
        await cadastroService.getAcolhidoPorMatricula(req.query).catch((error) => {
...
controller.get("/acolhidosPorUnidade", async (req, res) => {
    ...
    res.send(await cadastroService.getAcolhidosPorUnidade(req.query).catch((error) => {
        console.log(`Ocorreu um erro: ${error}`);
    }));
});
...
controller.delete("/deletarAcolhido", async (req, res) => {
    ...
    res.send(await cadastroService.deleteAcolhido(req.query).catch((error) => {
        console.log(`Ocorreu um erro: ${error}`);
    }));
});
...
controller.put("/atualizarAcolhido/:matricula", async (req, res) => {
    ...
    res.send(await cadastroService.updateAcolhido(req.params, req.body).catch((error) => {
        console.log(`Ocorreu um erro: ${error}`);
    }));
});
